    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (UserDao.validateUser(email, password)) {
                ...
                response.getWriter().write("{\"success\": true, \"redirectUrl\": \"home.jsp\",\"message\":\"Authenticated Successfully\"}");
            } else {
                response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
                response.getWriter().write("{\"success\":false,\"message\":\"Invalid email or password\"}");
            }
        } catch (DoesNotExistException e) {
            response.getWriter().write("{\"success\":false,\"message\":\"Account does not exist. Sign up first\"}");
            e.printStackTrace();
        }
